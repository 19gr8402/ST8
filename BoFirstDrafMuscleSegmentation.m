...
IOrg = Subject.Session.T2.right(:,:,3);
...
subplot(3,3,5), imshow(BWsdil), title('dilated gradient mask');
...
subplot(3,3,7), imshow(BWnobord), title('cleared border image');
...
subplot(3,3,8), imshow(BWsmooth), title('segmented image');

%% Remove too small objects

BWfinal = bwareaopen(BWsmooth, 5000);
subplot(3,3,9), imshow(BWfinal), title('cleared off small objects');
