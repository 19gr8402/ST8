function [P,J]=SnakeSJ(I,P,Options)
...
% Process inputs
defaultoptions=struct('Verbose',false,'nPoints',100,'Sigma2',20,'Alpha',0.2,'Beta',0.2,'Delta',0.1,'Gamma',1,'Kappa',2,'Iterations',100,'GIterations',0,'Mu',0.2,'Sigma3',1);
...
if(Options.Verbose)
    h=figure('units','normalized','outerposition',[0 0 1 1]); set(h,'render','opengl')
     subplot(2,2,1),
      imshow(I,[]); 
     ...
     subplot(2,2,3), 
      perPix = 3;
      [x,y]=ndgrid(1:perPix:size(Fext,1),1:perPix:size(Fext,2));
      imshow(Eext,[]), hold on; quiver(y,x,Fext(1:perPix:end,1:perPix:end,2)*2,Fext(1:perPix:end,1:perPix:end,1)*2,'AutoScale','off');
